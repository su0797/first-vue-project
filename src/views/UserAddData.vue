  ...
  <div class="container">
    ...
    <form class="need-validation" @submit.prevent="submitForm($event)" novalidate>
      <div v-for="(column, i) in columnList.data" :key="i">
        ...
        <UserSelectBox :label="column.meta_name" :selectValue="(inputValueList[column.meta_name] = selectValue)" @selectFromChild="inputValueList[column.meta_name] = $event.target.value" v-else-if="column.meta_type === '2'" />
        ...
        <UserRadioBox :label="column.meta_name" :radioValue="(inputValueList[column.meta_name] = radioValue)" @radioFromChild="inputValueList[column.meta_name] = $event.target.value" v-else-if="column.meta_type === '4'" />
      </div>
      <UserRadioBox :radioValue="(data_status = null)" @radioFromChild="data_status = $event.target.value" />
